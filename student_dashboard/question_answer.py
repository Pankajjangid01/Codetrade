    ...
    def add_answers_to_file(self):
        ...
        try:
            ...
            with open("answers.csv",'w') as answer_file:  # open the answer or creates it if it does not exist in write mode
                ...
                for answer in range(len(questionlines)):
                    ...
                    answer_file.write(f"{answer + 1}, {options[0]},  {options[1]}, {options[2]},  {options[3]}\n")  # store the options in the answer file 

            print("Answers Added Successfully")
        except Exception as exe:
            print(f"Something went wrong, try again later... :{str(exe)}")  # handle exception if any error occur in reading questions or writing answer to the file 
